...
import com.digitaltopology.myapp.config.KafkaProperties;
...
class DtCloudKafkaResourceIT {
    ...
    void setup() {
        ...
        DtCloudKafkaResource kafkaResource = new DtCloudKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/dt-cloud-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/dt-cloud-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
